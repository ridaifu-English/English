# Australia Training Camp - Lesson Plan
## Student-Led Research & Collaborative Learning

**Target Audience:** 30 students preparing for English training camp in Australia (March)
**Duration:** 6-8 lessons (45-60 minutes each)
**Learning Model:** Student-led collaborative research and Kahoot creation
**Objective:** To research, curate, and share knowledge about Australian culture through group collaboration

---

## Overview: Student-Led Research Approach

### Why Student-Led Learning?
Instead of traditional teacher lectures, students will:
- **Research** their own topics in groups
- **Curate** important information collaboratively
- **Create** Kahoot quizzes to teach other groups
- **Learn** from each other through gamification

### March Weather in Australia
**Important:** You're traveling during **late summer/early autumn** in Australia!
- **Season:** End of summer transitioning to autumn
- **Temperature:** 18-28°C (64-82°F) in most cities
- **Weather:** Still warm, less humid than peak summer
- **Perfect for:** Beach activities, outdoor exploration, sightseeing
- **Pack:** Light clothing, swimwear, sunscreen (UV still high!), light jacket for evenings

---

## Group Structure & Topic Assignments

### Group Formation (30 Students)
**Option A:** 6 groups of 5 students (skip one topic)
**Option B:** 5 groups of 6 students (skip two topics)
**Option C:** Mix of group sizes to cover all 7 topics

### Topic Assignments (7 Topics Available)

**Topics will be randomly assigned to groups. Students must decide what aspects of their topic are most important to research.**

**Group 1: Australian Geography and Weather**

**Group 2: Indigenous Australian Culture**

**Group 3: Australian Language and Communication**

**Group 4: Australian Food and Lifestyle**

**Group 5: Australian Sports and Recreation**

**Group 6: Living and Traveling in Australia**

**Group 7: Homestaying and Living with Host Families**

**Note for Students:** These topics are intentionally broad. Your group must:
- Decide what specific aspects to focus on
- Determine what information is most useful for the training camp
- Choose what your classmates should know about your topic

**Important:** Homestaying is very different from Japanese culture. Understanding host family expectations, communication, and daily life routines will be crucial for your trip!

---

## Group Roles & Responsibilities

### Role Assignments (5 students per group)

**Role 1: Research Leader (リサーチリーダー)**
- Finds reliable websites and sources
- Fact-checks information
- Ensures information is accurate and current
- Guides the team's research direction

**Role 2: Note-Taker (ノートテイカー)**
- Records important facts on paper
- Organizes information clearly
- Writes down key points from discussions
- Keeps track of interesting details

**Role 3: Table Organizer (テーブルオーガナイザー)**
- Writes questions in the planning table
- Ensures questions are clear and interesting
- Formats answer options properly
- Checks that questions test important knowledge

**Role 4: Kahoot Creator (Kahootクリエイター)**
- Inputs questions into Kahoot platform
- Adds images and visual elements
- Sets correct answers and time limits
- Tests the Kahoot before presenting

**Role 5: Quality Checker (品質チェッカー)**
- Reviews all work for accuracy
- Makes sure questions are not too easy or too hard
- Checks spelling and grammar
- Ensures the Kahoot will be fun and educational

---

## Research & Creation Workflow

### Session 1: Introduction & Research (60 minutes)

**Minutes 1-10: Introduction**
- Teacher explains project
- Form groups randomly
- **Randomly assign topics to each group** (draw from hat or use random number generator)
- Distribute worksheet and materials
- Assign roles within groups

**Minutes 11-30: Initial Research**
- Research Leader guides team to reliable sources
- All members search for information
- Note-Taker records key findings
- Focus: Find 12-15 interesting facts about your topic

**Minutes 31-50: Curate Information & Decide Focus**
- **First:** Discuss what aspects of your topic to focus on (What's most important for the trip?)
- Decide which facts are most useful for classmates
- Decide what other students NEED to know
- Select 10 facts to turn into questions
- Organize information by importance

**Minutes 51-60: Begin Planning Table**
- Table Organizer starts filling in the planning table
- Group decides on first 5 questions
- Quality Checker reviews for clarity

### Session 2: Question Creation (60 minutes)

**Minutes 1-20: Complete Planning Table**
- Finish writing all 10 questions
- Write 4 answer choices for each (1 correct, 3 wrong)
- Note which images or visuals would help
- Quality Checker reviews each question

**Minutes 21-40: Create Kahoot**
- Kahoot Creator inputs questions into platform
- Research Leader finds images online
- Other members help proofread
- Test questions as you go

**Minutes 41-60: Review & Polish**
- Play through the Kahoot as a group
- Check for mistakes
- Adjust difficulty if needed
- Prepare 2-minute introduction for presentation day

### Session 3: Presentations & Learning (60-70 minutes)

**Format for Each Group (10 minutes):**
- **Minutes 1-2:** Brief introduction - "Why this topic matters for Australia"
- **Minutes 3-8:** All students play the Kahoot (5-7 minutes)
- **Minutes 9-10:** Quick debrief - Share the most important takeaway

**All Groups Present:**
- Entire class learns from each other
- Everyone participates in all Kahoots
- Teacher can identify knowledge gaps
- Fun, competitive learning environment

**Note:** If using all 7 topics, presentations will take 70 minutes. Plan accordingly or split across two sessions.

---

## Research Guidelines

### Reliable Sources to Use

**Official Tourism & Government Sites:**
- australia.com (Official tourism website)
- bom.gov.au (Weather information)
- Indigenous.gov.au (Indigenous culture)

**Educational Sources:**
- Australian Geographic
- ABC Education
- BBC Travel - Australia
- National Geographic - Australia

**Cultural Information:**
- Australian slang dictionaries
- Australian history websites
- Cultural orientation guides

### What to Look For
- ✅ Recent information (2020s)
- ✅ Multiple sources confirming same fact
- ✅ Official or educational websites
- ✅ Practical, useful information for travelers
- ❌ Avoid: Blogs without sources, old information, stereotypes

### Research Tips
- Search in English for authentic Australian content
- Watch Australian YouTube videos for real examples
- Look for images that illustrate concepts
- Find facts that would surprise your classmates
- Focus on practical information you'll actually use

---

## Kahoot Requirements & Quality Standards

### Minimum Requirements
Each group's Kahoot must have:
- **10 questions** exactly
- **4 answer choices** per question (1 correct, 3 incorrect)
- **At least 5 images** throughout the quiz
- **Mix of question types:** Easy, medium, and challenging
- **Varied formats:** True/false, multiple choice, identification
- **Time limits:** 20-30 seconds per question (adjust based on difficulty)

### Quality Standards
- Questions must test IMPORTANT knowledge for the trip
- Wrong answers should be plausible (not obviously fake)
- Images should help illustrate concepts
- Language should be clear and simple
- No spelling or grammar errors
- Questions should be interesting and engaging

---

## Assessment & Grading

### Group Project Rubric (100 points)

**Research & Content Quality (40 points)**
- Information is accurate and well-researched (15 pts)
- Sources are reliable and current (10 pts)
- Content is relevant and useful for the trip (15 pts)

**Kahoot Quality (30 points)**
- Meets minimum requirements (10 questions, 5+ images) (10 pts)
- Questions are clear, interesting, and well-written (10 pts)
- Difficulty is appropriate and varied (10 pts)

**Collaboration & Process (20 points)**
- All members participated in their roles (10 pts)
- Planning table completed thoroughly (5 pts)
- Group worked well together (5 pts)

**Presentation & Delivery (10 points)**
- Clear 2-minute introduction (5 pts)
- Professional delivery and enthusiasm (5 pts)

### Individual Participation (Bonus Points)
- Active participation in other groups' Kahoots (up to 10 bonus points)
- Top 3 scores across all Kahoots receive recognition

---

## Additional Resources

### Recommended Viewing
- **Movies:** Finding Nemo, Bran Nue Dae, The Sapphires, Red Dog
- **TV Shows:** Bluey, Bondi Rescue, Outback Truckers
- **YouTube Channels:** Aussie World, Australian Geographic, Visit Australia

### Useful Apps for Students
- **Google Maps:** Navigation
- **BOM Weather:** Australian weather forecasts
- **Translate:** Quick translations
- **Kahoot:** Create and play quizzes

### Starting Points for Research (Optional)

**Important:** These are just suggestions. Your group should search for information based on what YOU think is important!

**Group 1 (Geography & Weather):**
- australia.com
- bom.gov.au (Bureau of Meteorology)

**Group 2 (Indigenous Culture):**
- indigenous.gov.au
- australianstogether.org.au

**Group 3 (Language & Communication):**
- Search: "Australian English"
- Search: "Australian communication style"

**Group 4 (Food & Lifestyle):**
- australia.com
- Search: "Australian culture"

**Group 5 (Sports & Recreation):**
- australia.com
- Search: "Australian sports"

**Group 6 (Living & Traveling):**
- australia.gov.au
- Search: "travel Australia tips"

**Group 7 (Homestaying):**
- Search: "Australian homestay culture"
- Search: "host family etiquette Australia"
- Search: "living with Australian families"

---

## Teacher Notes

### Facilitation Tips
- **Circulate during research:** Help groups find good sources
- **Check planning tables:** Review before they create Kahoots
- **Test Kahoots:** Play through each one before presentation day
- **Manage time:** Use timers to keep groups on track
- **Encourage collaboration:** Students should discuss and debate

### Common Challenges & Solutions

**Challenge:** Students choosing unreliable sources
- **Solution:** Provide list of approved websites, teach how to verify sources

**Challenge:** Questions too easy or too difficult
- **Solution:** Quality Checker role is crucial; teacher reviews planning tables

**Challenge:** Unequal participation
- **Solution:** Clear role assignments; rotate computer/paper tasks

**Challenge:** Running out of time
- **Solution:** Assign some research as homework; prioritize completing 10 questions

### Differentiation Strategies
- **Stronger students:** Can be Research Leaders or Quality Checkers
- **Visual learners:** Focus on finding images, creating visual questions
- **Students who need support:** Pair with helpful teammates, assign Note-Taker role
- **Advanced English:** Can help with question writing and proofreading

---

## Project Timeline

### Week 1-2: Teacher Introduction
- Show example Kahoots
- Explain project requirements
- Demonstrate research process
- Practice finding reliable sources

### Week 3: Session 1 - Research & Planning
- Form groups randomly
- **Randomly assign topics** to each group
- Distribute worksheets
- Conduct research
- Begin planning table

### Week 4: Session 2 - Kahoot Creation
- Complete planning tables
- Create Kahoots on computers
- Add images and format questions
- Teacher reviews and provides feedback

### Week 5: Session 3 - Presentations
- All groups present (60-70 minutes depending on number of groups)
- Students play all Kahoots
- Discussion and reflection
- Consider splitting presentations across two sessions if using all 7 topics

### Week 6-8: Review & Preparation
- Review areas where Kahoots showed weakness
- Practice role-plays and scenarios
- Final trip preparation
- Packing and logistics

---

## Learning Outcomes

By the end of this project, students will be able to:
- **Research effectively** using reliable English sources
- **Curate information** and identify what's important
- **Collaborate** with teammates on a complex project
- **Create educational content** for peers
- **Understand Australian culture** across 6 key areas
- **Communicate confidently** about Australia in English
- **Feel prepared and excited** for the training camp

---

## Conclusion

This student-led approach ensures:
- ✅ Active learning instead of passive listening
- ✅ Deep understanding through teaching others
- ✅ Collaboration and teamwork skills
- ✅ Technology integration and digital literacy
- ✅ Comprehensive coverage of Australian culture
- ✅ Engagement and motivation through gamification
- ✅ Preparation for real-world travel situations

**Final Message to Students:**
You are the experts! Through your research and Kahoots, you'll teach each other everything needed for an amazing Australia training camp. Work together, have fun, and get ready for an unforgettable experience!

**As Australians say: "She'll be right, mate!"** (Everything will be okay, friend!)

---

## Quick Reference Guide

### Essential Phrases
| English | Australian |
|---------|-----------|
| Hello | G'day |
| Thank you | Thanks / Ta / Cheers |
| You're welcome | No worries |
| Excuse me | Sorry / Scuse me |
| How are you? | How ya goin'? |
| See you later | See ya / Catch ya later |
| That's great | Too easy / Ripper |

### Emergency Contacts Template
*Students should fill this out before departure:*

- Emergency Services: 000
- Embassy/Consulate: ________________
- Emergency Contact at Home: ________________
- Group Leader: ________________
- Accommodation Address: ________________
- Travel Insurance: ________________

### Important Reminders
🌞 Wear sunscreen daily
💧 Drink lots of water
🏊 Swim between the flags
📱 Keep emergency numbers handy
🗣️ Don't be afraid to ask for help
🙂 Smile and enjoy the experience!

---

**Good luck with your Australia Training Camp preparation!**
